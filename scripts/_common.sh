...
if [ "$DEBUG" = 1 ]; then
    debug() {
        ...
        # Aditionally, we limit the string length to 19, and add
        # up to 20 spaces on the right
...
# Safe curl wrapper that blocks any protocols other than HTTP(s).
# You should use this instead of regular curl when dealing with
# user-provided URLs
fetch() {
    curl -fL \
        --proto =http,https \
